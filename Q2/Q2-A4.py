...
def paresingxml():
	count = 0
	with open('Twi.txt', 'r') as searchfile:
		with open('freindstwit.txt','w') as wf:
...
def extractingnumber():
	with open('Twi.txt','r') as fc ,open('numberstwi.txt ','w') as nc:
			for line in fc:
...
def calculateMean():
	lis=[]
	with open ('numberstwi.txt ','r') as nc:
		total = sum(int(x)
		for line in nc
		for x in line.split())
		#print ("Total = ",total)
